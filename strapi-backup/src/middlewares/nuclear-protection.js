        ...
        try {
          ...
          if (pathParts.length >= 2 && pathParts[0] === 'api') {
            ...
            if (validCollections.includes(collectionName)) {
              ...
              if (itemId) {
                console.log('🛡️ NUCLEAR: Protegendo', collectionName, 'ID', itemId);
                ...
                try {
                  ...
                  await strapi.db.connection.raw(
                    `INSERT INTO strapi_permissions (action, role, enabled, created_at, updated_at) 
                     VALUES
                       ('api::${singular}.${singular}.find', 2, true, NOW(), NOW()),
                       ('api::${singular}.${singular}.findOne', 2, true, NOW(), NOW())
                ...
                console.log('✅ NUCLEAR:', collectionName, 'protegido com sucesso');
              }
            }
          }
        } catch (error) {
        ...
        if (valid && ctx.response.status === 403) {
          ...
          setTimeout(async () => {
            try {
              ...
              console.log('🛡️ Nuclear: Permissões ativadas para', collection, 'após 403');
            } catch (e) {
              // Silencioso
            }
          }, 100);
